...
versions = ["DirectPartition_Restart", "DirectPartition_HitPoint"]
exec_funcs = {
    "DirectPartition_Restart": lambda graphs: perform_direct_tree_attempt(graphs[1], multi_walk_start_behavior=GraphNav.MultiWalkStartBehavior.RESTART),
    "DirectPartition_HitPoint": lambda graphs: perform_direct_tree_attempt(graphs[1], multi_walk_start_behavior=GraphNav.MultiWalkStartBehavior.HIT_POINT),
}
n_sizes = [256, 1024, 1764, 2500, 3136, 3844, 4624]
reps = 1000

def perform_direct_tree_attempt(data: TwinGraph, multi_walk_start_behavior: GraphNav.MultiWalkStartBehavior):
    region_tree = RegionTree(data, epsilon=0.05)
    # Using default start_selection_method=WALK as it is generally robust
    graph_nav = GraphNav(
        data, 
        region_tree, 
        start_selection_method=GraphNav.StartSelectionMethod.WALK, 
        multi_walk_start_behavior=multi_walk_start_behavior
    )
...
def bench():
    with keep.running(): # Prevent sleep during benchmarking
        ...
        for n in n_sizes:
            ...
            # Save results
            df.to_csv("benchmarking_results_gstabp_multi_walk_start.csv")
            print("Saved benchmarking results to benchmarking_results_gstabp_multi_walk_start.csv")
